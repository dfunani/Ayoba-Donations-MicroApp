...
window.onload = async(event) => {
  ...
  CHARITIES = await fetch("https://devstrapi.thedigitalacademy.co.za/api/ayoba-donations-app-charities").then(res => res.json()).then(result => result.data).catch(() => []) 
...
const searchNPO = (event) => {
  ...
  CHARITIES.filter(term => reg.test(term.attributes.title)).forEach((element) => {
    scroller.innerHTML += createCharity(element.attributes);
  });
}
...
const createCharityModal = (obj) => {
  ...
  <div class="card-body border-0 d-flex flex-column desc">
    <h5 class="card-title h5-title fs-1 mt-3" id="title_${obj.title}">${obj.title}</h5>
    <p class="card-text fs-5" id="card_${obj.title}">${description(obj.title, obj.description)}</p>
    <p class="card-text">
...
const description = (title, description) => {
  ...
  return description.split(".").slice(0, 2).join(".") + `.<br/><a onclick="learnMore('${title}', '${description.split("\n").join(" ")}')">learn more ></a>`
}

const learnMore = (title, description) => {
  const desc = document.getElementById("card_" + title)
  desc.innerHTML = `<div class="border bg-light w-25 m-auto" onclick="truncate('${title}', '${description}')"><svg width="12" height="8" viewBox="0 0 12 8" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const createDonationOverlay = (obj) => {
  return `<div class="fixed-bottom" id="fixed_${obj.title}" hidden>
  ...
  <div class="input-select input-currency-select">
      <div class="dropdown">
        <a class="dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false">
          <img src="src/assets/Icon-Flag-America.png" class="dropdown_currency" alt="...">
        </a>
      ...
      <div class="input-currency">
          ...
          <input placeholder="R" type="number" data-currency="ZAR" name="currency" id="currency" step="0.01">
      </div></div>   
  ...
  <button type="submit" id="payment_${
    obj.title
  }" class="fs-5 text-light border-0 donate fixed-bottom" onclick="startPaymentOverlay()">Continue to payments</button>
  </div>    
  </div>
</div>`
}
...
const setCurrency = (icon, country, name, code) => {
  const currencyDropdown = document.querySelector(".dropdown_currency")
  const inputTag = document.querySelector(".tag")
  const inputBox = document.getElementById("currency")
...
const getCurrency = () => {
  return CURRENCIES.map((elem) => `<li onclick="setCurrency('${elem.icon}', '${elem.country}', '${elem.currency.name}', '${elem.currency.code}')"><a class="dropdown-item" href="#">${elem.country}</a></li>`).join("")
}
